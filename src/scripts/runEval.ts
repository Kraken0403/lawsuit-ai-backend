...
import type { EvalCase, EvalRunReport } from "../eval/types.js";
...
async function importFirst(candidates: string[], exportName: string, required = true) {
  ...
  if (!required) return { fn: null, source: null };

  throw new Error(
    `Could not find export "${exportName}". Tried:\n${candidates.join("\n")}\n\nUpdate the candidate paths in src/scripts/runEval.ts.`
  );
}
...
function summarize(report: EvalRunReport) {
  const bucketCounts = new Map<string, number>();

  for (const result of report.results) {
    bucketCounts.set(
      result.failureBucket,
      (bucketCounts.get(result.failureBucket) || 0) + 1
    );
  }
  ...
  if (failed.length) {
    ...
    for (const result of failed) {
      console.log(
        `- ${result.id} | ${result.failureBucket} | ${result.query} | top=${result.actual.topTitle || "NONE"} | latency=${formatMs(result.latencyMs)}`
      );
    }
  }
}
...
async function main() {
  ...
  const { fn: orchestrateSearch, source: orchestratorSource } = await importFirst(
    ORCHESTRATOR_IMPORT_CANDIDATES,
    "orchestrateSearch",
    true
  );

  const { fn: composeAnswer, source: composeSource } = await importFirst(
    COMPOSE_IMPORT_CANDIDATES,
    "composeAnswer",
    false
  );
  ...
  for (const def of cases) {
    ...
    try {
      ...
      console.log(
        `[${scored.pass ? "PASS" : "FAIL"}] ${def.id} | ${def.query} | ${formatMs(latencyMs)} | top=${scored.actual.topTitle || "NONE"}`
      );
    } catch (error: any) {
      const latencyMs = Date.now() - started;
