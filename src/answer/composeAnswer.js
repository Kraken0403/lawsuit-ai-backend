...
function fallbackAnswer(searchResult) {
  ...
  if (
    traceNotes.some((note) =>
      String(note || "").toLowerCase().includes("latest/recent")
    )
  ) {
    warnings.push(
      "Recency-aware retrieval is still heuristic because decision dates are parsed at runtime from text metadata."
    );
  }
...
function buildMetadataAnswer(searchResult) {
  ...
  const summary = `${top?.title || "This case"} ${metadataLabel(field)}: ${
...
function buildTopFiveCitations(groupedCases) {
  ...
  for (const group of (groupedCases || []).slice(0, 5)) {
    ...
    if (chunk) out.push(makeCitation(group, chunk));
  }
...
function reorderGroupedCases(groupedCases = [], searchResult = {}) {
  ...
  if (!hasMeaningfulSignal) return groupedCases;
...
function inferCourtCodeFromPayload(payload) {
  ...
  if (fileName.startsWith("1")) return "SC";
  if (fileName.startsWith("2")) return "DELHI_HC";
  if (fileName.startsWith("3")) return "BOMBAY_HC";
  if (fileName.startsWith("4")) return "GUJARAT_HC";

  if (court.includes("supreme court")) return "SC";
  if (court.includes("delhi")) return "DELHI_HC";
  if (court.includes("bombay")) return "BOMBAY_HC";
  if (court.includes("gujarat")) return "GUJARAT_HC";
...
function latestRetrievalPenalty(searchResult, warningsOut) {
  const query = searchResult?.query || {};
  if (query.intent !== "latest_cases") return 0;
  ...
  if (!groupedCases.length) return 0.18;
  ...
  const newestTime = Math.max(
    ...groupedCases
      .map((group) =>
        Math.max(
          0,
          ...((group.chunks || []).map((c) => parseDecisionTime(c.payload || {}) || 0))
        )
      )
      .filter(Boolean)
  );
  ...
  for (const group of groupedCases.slice(0, 5)) {
    ...
    if (courts.length) {
      const courtOk = courts.every((court) => {
        if (court.includes("supreme")) return codes.has("SC") || blob.includes("supreme court");
        if (court.includes("delhi")) return codes.has("DELHI_HC") || blob.includes("delhi");
        if (court.includes("bombay")) return codes.has("BOMBAY_HC") || blob.includes("bombay");
        if (court.includes("gujarat")) return codes.has("GUJARAT_HC") || blob.includes("gujarat");
        return blob.includes(court);
      });

      if (!courtOk) penalty += 0.06;
    }

    if (jurisdictions.length) {
      const jurisdictionOk = jurisdictions.every((j) => blob.includes(j));
      if (!jurisdictionOk) penalty += 0.05;
    }

    if (subjects.length) {
      const subjectOk = subjects.some((s) => blob.includes(s));
      if (!subjectOk) penalty += 0.05;
    }
  ...
  if (penalty >= 0.12) {
    warningsOut.push(
      "Latest/recent retrieval still includes mixed or older results because decision dates are parsed heuristically from metadata strings."
    );
  }
...
function citationResolutionPenalty(searchResult, warningsOut) {
 ...
 const exactResolved = notes.some((note) => {
  const n = String(note || "").toLowerCase();
  return n.includes("resolved exact citation");
});

  const aliasResolved = notes.some((note) =>
    String(note || "").toLowerCase().includes("citation alias llm recovery resolved")
  );

  if (!exactResolved && !aliasResolved) {
    if (
      notes.some((note) =>
        String(note || "").toLowerCase().includes("citation-first resolution found no exact citation match")
      )
    ) {
      penalty += 0.18;
      warningsOut.push(
        "Direct citation resolution was not exact; related case cards may be suppressed to avoid showing the wrong case."
      );
    }
  }
...
function shouldSuppressCaseCards(searchResult, llmText) {
  const notes = Array.isArray(searchResult?.trace?.notes) ? searchResult.trace.notes : [];
  const hasCitationQuery =
    Array.isArray(searchResult?.query?.citations) && searchResult.query.citations.length > 0;

  if (!hasCitationQuery) return false;

  const exactResolved = notes.some((note) =>
    String(note || "").toLowerCase().includes("resolved exact citation and expanded full case")
  );
  ...
  if (exactResolved || aliasResolved) return false;

  const failedResolution = notes.some((note) => {
    const n = String(note || "").toLowerCase();
    return (
      n.includes("citation-first resolution found no exact citation match") ||
      n.includes("citation alias recovery found no resolvable") ||
      n.includes("citation alias recovery extracted")
    );
  });

  if (failedResolution) return true;

  if (
    typeof llmText === "string" &&
    /cannot reliably|do not identify|cannot identify|could not identify/i.test(llmText)
  ) {
    return true;
  }
...
function deriveWarnings(searchResult, llmText) {
  ...
  if (!llmText) {
    warnings.push(
      "LLM generation was unavailable, so a deterministic fallback was used."
    );
  }

  if (
    traceNotes.some((note) =>
      String(note || "").toLowerCase().includes("router output was weak")
    )
  ) {
  ...
  if (
    traceNotes.some((note) =>
      String(note || "").toLowerCase().includes("latest/recent case query")
    )
  ) {
    warnings.push(
      "Recency-aware retrieval is heuristic because decision dates are parsed at runtime from metadata strings."
    );
  }
  ...
  if (shouldSuppressCaseCards(searchResult, llmText)) {
    warnings.push(
      "Related case cards were hidden because citation resolution did not confidently identify the actual case."
    );
  }
...
function deriveConfidence(searchResult, llmText, groupedCases) {
  ...
  if (
    Array.isArray(searchResult?.trace?.notes) &&
    searchResult.trace.notes.some((note) =>
...
export async function composeAnswer(searchResult) {
  ...
  if (groupedCases.length === 1) {
    ...
    const digest = buildDigest(
      top,
      summary,
      llmCitations.length ? llmCitations : undefined
    );

    return {
      answerType: "hybrid_answer",
      summary,
      caseDigests: suppressCaseCards ? [] : [digest],
      citations: suppressCaseCards
        ? []
        : llmCitations.length
        ? llmCitations
        : digest.citations,
  ...
  return {
    answerType: "hybrid_answer",
    summary,
    caseDigests: suppressCaseCards ? [] : buildTopFiveDigests(groupedCases),
    citations: suppressCaseCards
      ? []
      : llmCitations.length
      ? llmCitations
