function compact(text) {
  ...
  function getParaStart(chunk) {
    if (chunk?.paragraphStart != null) return Number(chunk.paragraphStart);
    const raw = chunk?.payload?.paragraphStart;
    return raw != null ? Number(raw) : 0;
  }
  
  function getParaEnd(chunk) {
    if (chunk?.paragraphEnd != null) return Number(chunk.paragraphEnd);
  ...
  function scoreChunk(chunk, query, totalChunks) {
    ...
    if (idx <= 1) score += 10;
    if (idx >= Math.max(0, totalChunks - 5)) score += 18;
  
    // Later paras often contain more dispositive material
    score += Math.min(15, paraStart / 180);
  
    if (style === "holding") {
      if (operativeRegex().test(text)) score += 65;
      if (issueRegex(query).test(text)) score += 12;
    } else if (style === "summary") {
      if (issueRegex(query).test(text)) score += 22;
      if (operativeRegex().test(text)) score += 28;
    } else {
      if (issueRegex(query).test(text)) score += 14;
      if (operativeRegex().test(text)) score += 18;
    }
  ...
  export function selectSingleCaseEvidence(group, query) {
    ...
    const selectedChunks = uniqueChunks([...intro, ...topScored, ...tail]).sort(
      (a, b) => getChunkIndex(a) - getChunkIndex(b)
    );
  
    const supportingCitations = uniqueCitations(
      ranked
        .slice(0, 5)
