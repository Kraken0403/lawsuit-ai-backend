...
function citationResolutionPenalty(searchResult, warningsOut) {
  ...
  if (
    ...
    penalty += 0.18;
    warningsOut.push(
      "Direct citation resolution was not exact; the system used secondary evidence or citation-alias recovery."
    );
  }

  if (
    notes.some((note) =>
      String(note || "").toLowerCase().includes("recovered citation alias")
    )
  ) {
    penalty += 0.06;
  }
