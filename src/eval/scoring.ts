...
function includesLoose(haystack: string | null | undefined, needle: string | null | undefined): boolean {
...
function chooseFailureBucket(params: {
  searchResult: any;
  answer: any;
  checks: EvalCaseResult["checks"];
  latencyMs: number;
  maxLatencyMs?: number;
}): EvalCaseResult["failureBucket"] {
...
export function scoreEvalCase(params: {
  ...
  if (def.expectation.topCitationIncludes) {
    checks.topCitationIncludes = includesLoose(
      String(top?.citation || ""),
      def.expectation.topCitationIncludes
    );
  }

  if (def.expectation.topTitleIncludes) {
    checks.topTitleIncludes = includesLoose(
      String(top?.title || ""),
      def.expectation.topTitleIncludes
    );
  }
  ...
  if (Array.isArray(def.expectation.answerMustContain) && def.expectation.answerMustContain.length) {
    checks.answerMustContain = def.expectation.answerMustContain.every((piece) =>
      includesLoose(summary, piece)
    );
  }

  if (Array.isArray(def.expectation.answerMustNotContain) && def.expectation.answerMustNotContain.length) {
    checks.answerMustNotContain = def.expectation.answerMustNotContain.every(
      (piece) => !includesLoose(summary, piece)
    );
  }
