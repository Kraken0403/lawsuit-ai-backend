...
function getChunkText(payload: Record<string, any>) {
  return (
...
function buildCaseFilter(caseIdOrFileName: string | number) {
  const raw = String(caseIdOrFileName).trim();
  const numeric = Number(raw);
  const values: Array<string | number> = [raw];
...
async function scrollAllCasePoints(caseIdOrFileName: string | number) {
  if (!QDRANT_URL) {
  ...
  for (let i = 0; i < 200; i += 1) {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Qdrant scroll failed with status ${response.status}: ${text}`
      );
    }
...
export async function fetchFullCaseFromQdrant(
  caseIdOrFileName: string | number
): Promise<QdrantFullCase> {
  const points = await scrollAllCasePoints(caseIdOrFileName);
  const chunks = dedupeAndSortChunks(points);

  if (!chunks.length) {
    throw new Error(
      `No Qdrant chunks found for caseId/fileName=${caseIdOrFileName}`
    );
  }
