...
function normalizeAllowedCourtsFromArray(value: unknown[]): SsoAllowedCourt[] {
  const courts: SsoAllowedCourt[] = [];
  ...
  for (const item of value) {
    ...
    const subtitle =
      typeof raw.subtitle === "string" ? raw.subtitle.trim() : undefined;
...
function normalizeAllowedCourts(value: unknown): SsoAllowedCourt[] {
...
function uniqueAllowedCourtIds(courts: SsoAllowedCourt[]) {
  return [...new Set(courts.map((court) => court.subid).filter((id): id is number => id != null))];
}
...
function deriveExternalUserId(decoded: jwt.JwtPayload) {
...
export function verifyHs256SsoToken(rawToken: string): SsoTokenClaims {
  ...
  if (!externalUserId) {
    throw new Error(
      "Invalid SSO token: missing sub/externalUserId/username/email."
    );
  }
  ...
  return {
    ...
    username:
      typeof decoded.username === "string" ? decoded.username.trim() : undefined,
    email:
    ...
    hasAiAccess:
      typeof decoded.hasAiAccess === "boolean"
        ? decoded.hasAiAccess
