...
function stripHtmlToCaseText(html: string) {
  return String(html || "")
    .replace(/<script\b[\s\S]*?<\/script>/gi, " ")
    .replace(/<style\b[\s\S]*?<\/style>/gi, " ")
    .replace(/<(br|hr)\s*\/?>/gi, "\n")
    .replace(
      /<\/(p|div|section|article|li|tr|table|h1|h2|h3|h4|h5|h6)>/gi,
      "\n"
    )
    .replace(/<li\b[^>]*>/gi, "- ")
    .replace(/<[^>]+>/g, " ")
...
function normalizeCaseText(text: string) {
  return String(text || "")
    .replace(/\r/g, "")
    .replace(/[ \t]+\n/g, "\n")
    .replace(/\n[ \t]+/g, "\n")
...
function isUsableCaseText(text: string, minLength = 1200) {
...
function buildFocusedSummaryInput(text: string, maxChars = 32000) {
  ...
  const scored = paragraphs.map((paragraph, index) => {
    ...
    if (index < 10) score += 4;
    if (index >= paragraphs.length - 10) score += 5;
  ...
  const take = (items: typeof scored) => {
    for (const item of items) {
      if (used.has(item.index)) continue;
      if (total + item.paragraph.length + 2 > maxChars) continue;
...
function splitIntoStreamChunks(text: string, size = 140) {
  const chunks: string[] = [];
  let index = 0;
...
async function streamDetailedSummaryMarkdownFromText(
  input: {
    title: string;
    citation: string;
    court: string;
...
async function prepareDetailedSummarySource(
...
export async function streamDetailedCaseSummary(
  caseId: string | number,
  options: {
    html?: string | null;
  } | undefined,
