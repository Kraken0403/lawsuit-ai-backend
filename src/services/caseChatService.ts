...
import {
  getLatestDetailedCaseSummary,
  getOrCreateDetailedCaseSummary,
} from "./caseSummaryService.js";
...
function formatChunkLabel(chunk: {
  ...
  if (chunk.chunkIndex != null) parts.push(`chunk ${chunk.chunkIndex}`);
...
export async function askCaseOnlyChat(
  ...
  const detailedSummary =
    existingSummary || (await getOrCreateDetailedCaseSummary(caseId)).summary;
