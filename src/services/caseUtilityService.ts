...
function sanitizeHtmlForPdf(html: string) {
  return String(html || "")
...
function getSmtpTransporter() {
  ...
  if (!host) {
    throw Object.assign(
      new Error("SMTP_HOST is not configured on the backend."),
      { status: 500 }
    );
  }
    ...
    messages: [
      {
        role: "system",
        content:
          "You are a legal translation assistant. Translate the provided Indian legal case text accurately. Preserve party names, court names, citations, section numbers, article numbers, dates, legal terms, formatting, and paragraph structure as much as possible. Do not add commentary. Only return the translated text.",
      },
      {
        role: "user",
        content: [
...
export async function sendCasePdfEmail(params: {
  ...
  const info = await transporter.sendMail({
    ...
    html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#111827;white-space:pre-wrap;">${escapeHtml(
      body || "Please find the attached case PDF."
    )}</div>`,
    attachments: [
      {
        filename,
