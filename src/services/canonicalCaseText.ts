...
import type { QdrantFullCase } from "./qdrantCaseService.js";

function normalizeWhitespace(text: string) {
  return text.replace(/\r\n/g, "\n").replace(/[ \t]+/g, " ").trim();
}

function findSuffixPrefixOverlap(a: string, b: string, minOverlap = 120) {
...
export function buildCanonicalCaseTextFromQdrant(fullCase: QdrantFullCase) {
  ...
  let merged = "";
  const seenExactChunks = new Set<string>();
