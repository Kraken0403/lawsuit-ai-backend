...
function resolveEnvPath(filePath: string): string {
...
function read(primaryName: string, fallbackName?: string): string {
...
function setDerivedSqlServerDatabaseUrl() {
  ...
  const trustServerCertificate =
    read("DATABASE_TRUST_SERVER_CERT", "SQL_TRUST_SERVER_CERT") || "true";
...
function getDatabaseNameFromSqlServerUrl(value: unknown): string {
...
function assertDatabaseUrlMatchesDatabaseName() {
  const databaseUrlName = getDatabaseNameFromSqlServerUrl(
    process.env.DATABASE_URL
  );
  ...
  if (
    ...
    throw new Error(
      `DATABASE_URL database (${databaseUrlName}) does not match DATABASE_NAME (${databaseName}). Refusing to continue.`
    );
  }
}
