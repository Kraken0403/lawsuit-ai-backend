...
function expandOriginJurisdictionTerms(origins: string[] = []): string[] {
  const out: string[] = [];
...
function expandLowerCourtHints(lowerCourtHints: string[] = []): string[] {
  const out: string[] = [];
...
function buildIssueBoostTerms(query: ClassifiedQuery): string[] {
  const q = normalizeLoose(query.normalizedQuery || query.originalQuery || "");
  const filters = query.filters || {};
  const out: string[] = [];

  if (
    /\bnon[\s-]?existent\b|\bdoes not exist\b|\bfabricated\b|\bfalse citation\b|\bfake citation\b|\bfake judgment\b|\bforged\b|\bfraud on court\b|\bbogus\b|\bimaginary\b/.test(
      q
    )
  ) {
    out.push(
      "fake Supreme Court judgment",
      "fabricated Supreme Court citation",
      "false citation",
