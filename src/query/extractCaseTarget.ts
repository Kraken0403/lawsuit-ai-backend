...
function looksLikeCaseName(text: string): boolean {
...
function rejectTopicalPhrase(text: string): boolean {
  return /\b(doctrine|evolution|history|development|jurisprudence|principle|constitutional framework|basic structure)\b/i.test(
    text
  );
}

export function extractCaseTarget(query: string): string | null {
  ...
  if (!q) return null;
  if (rejectTopicalPhrase(q)) return null;
  if (!looksLikeCaseName(q)) return null;
