...
type ExportBranding = {
  ...
  letterheadHeightPx?: number | null;
...
function buildDocxBodyHtml(bodyHtml: string) {
  return `<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8" />
...
function buildDocxHeaderFooterHtml(branding?: ExportBranding | null) {
...
async function normalizeToBuffer(data: ArrayBuffer | Blob | Buffer) {
...
export async function generateDraftDocxBuffer({
  title,
  bodyHtml,
  branding,
}: {
  ...
  const { headerHtml, footerHtml, headerHeightPx, footerHeightPx } =
    buildDocxHeaderFooterHtml(branding);
