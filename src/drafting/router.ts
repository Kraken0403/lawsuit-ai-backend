...
const FAMILY_KEYWORDS: Record<DocumentFamily, string[]> = {
...
function inferFamilyFromQuery(query: string): DocumentFamily | null {
  const q = normalizeText(query);

  if (
    q.includes("service agreement") ||
...
function directUseAttachmentRequested(query: string) {
  const q = normalizeText(query);

  return (
    q.includes("use attached") ||
...
function inferIntent(query: string, hasAttachments: boolean): DraftingIntent {
  const q = normalizeText(query);
...
function inferTone(query: string): DraftingTone {
  const q = normalizeText(query);

  if (q.includes("aggressive") || q.includes("strongly worded")) return "aggressive";
  if (q.includes("strict")) return "strict";
  if (q.includes("formal")) return "formal";

  return "neutral";
}

function collectRecentUserContext(messages: ChatTurn[] = [], query = ""): string {
...
function hasEnoughFacts(
  ...
  for (const check of checks) {
    if (check.test(combined)) signalCount += 1;
  }

  return combined.length >= 220 && signalCount >= 3;
}

function getGenericFields(family: DocumentFamily | null): string[] {
  switch (family) {
    case "notice":
      return [
        "sender_details",
...
function getFieldsFromTopTemplate(
...
function factSatisfiesField(field: string, facts: DraftingFieldMap) {
  if (facts[field]) return true;

  const aliases = FIELD_ALIASES[field] || [];
  return aliases.some((alias) => !!facts[alias]);
}

export async function routeDraftingQuery({
  userId,
  query,
  messages = [],
  attachments = [],
}: RouteInput): Promise<DraftingPlan> {
  ...
  const explicitAttachmentOverride =
    attachments.length > 0 && directUseAttachmentRequested(query);

  const resolvedQuery =
    compact(routerState.normalizedUserBrief) ||
  ...
  let resolvedIntent: DraftingIntent = inferIntent(
    resolvedQuery,
    attachments.length > 0
  );

  let matchLevel: DraftingPlan["matchLevel"] = "none";
  ...
  let strategy: DraftingPlan["strategy"] =
    attachments.length > 0
  ...
  if (
    ...
    (explicitAttachmentOverride ||
      /use (the )?(attached|uploaded) format|same format|same structure|based on attached/i.test(
        resolvedQuery
      ))
  ) {
  ...
  const detectedFamily =
    resolvedFamily || ((top?.family as DocumentFamily | undefined) ?? null);
  ...
  const requiredFields = templateFields.length
    ? templateFields
    : getGenericFields((top?.family as DocumentFamily | null) || detectedFamily);

  const extractedFacts = routerState.extractedFacts || {};
  const missingFields = uniqueStrings(
    (routerState.missingFacts || []).concat(
      requiredFields.filter((field) => !factSatisfiesField(field, extractedFacts))
    )
  );

  const enoughFactsNow =
    routerState.shouldGenerateNow ||
  ...
  if (top) {
    reasoningNotes.push(
      `top template candidate: ${top.title} (${top.family}${
        top.subtype ? ` / ${top.subtype}` : ""
      })`
    );
