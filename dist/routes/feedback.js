...
function normalizeAssistantReaction(value) {
...
function normalizeOptionalString(value, maxLength = 5000) {
    if (value == null)
        return null;
...
function normalizeMode(value) {
    const normalized = String(value ?? "").trim().toLowerCase();
    if (normalized === "case_modal")
        return "case_modal";
    if (normalized === "judgment_mode")
        return "judgment_mode";
    if (normalized === "drafting_mode")
        return "drafting_mode";
    return null;
}
feedbackRouter.get("/assistant-message/:assistantMessageId", async (req, res, next) => {
    try {
...
feedbackRouter.post("/assistant-message", async (req, res, next) => {
    try {
        const raw = req.body || {};
