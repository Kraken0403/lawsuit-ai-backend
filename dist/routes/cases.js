...
function normalizeCaseFeedback(value) {
...
casesRouter.post("/:caseId/summary/detailed", async (req, res, next) => {
    try {
...
casesRouter.post("/:caseId/summary/detailed/stream", async (req, res, next) => {
...
casesRouter.post("/:caseId/chat/stream", async (req, res, next) => {
    let clientClosed = false;
    const markClosed = () => {
        clientClosed = true;
    };
...
casesRouter.get("/:caseId/feedback", async (req, res, next) => {
    try {
        const caseId = req.params.caseId;
        const assistantMessageId = normalizeOptionalString(req.query.assistantMessageId, 191);
...
casesRouter.post("/:caseId/feedback", async (req, res, next) => {
    try {
        const caseId = req.params.caseId;
        const raw = req.body || {};
