...
async function resolveOwnedConversationId(req, conversationId) {
...
const uploadSingleImage = draftingSettingsAssetUpload.single("file");
...
const speechUpload = multer({
    ...
    fileFilter: (_req, file, cb) => {
        if (!ALLOWED_AUDIO_MIME_TYPES.has(file.mimetype)) {
            return cb(new Error("Only WEBM, MP4, MP3, WAV, OGG, M4A, or AAC audio files are allowed."));
        }
        cb(null, true);
    },
}).single("file");
function normalizeDraftBrandingMode(value) {
...
function setNullableStringField(target, body, key) {
    if (Object.prototype.hasOwnProperty.call(body, key)) {
        target[key] = compact(body?.[key]) || null;
    }
}
function buildFirmSettingsPayload(body) {
    const payload = {};
    ...
    if (Object.prototype.hasOwnProperty.call(body, "draftingBrandingMode")) {
        payload.draftingBrandingMode = normalizeDraftBrandingMode(body?.draftingBrandingMode);
    }
    ...
    if (Object.prototype.hasOwnProperty.call(body, "draftingLetterheadHeightPx")) {
        payload.draftingLetterheadHeightPx = parseNullableInt(body?.draftingLetterheadHeightPx);
    }
    ...
    if (Object.prototype.hasOwnProperty.call(body, "draftingDefaultJurisdiction")) {
...
draftingRouter.get("/settings", async (req, res, next) => {
    try {
...
draftingRouter.put("/settings", async (req, res, next) => {
    try {
...
draftingRouter.post("/settings/assets/:kind", uploadSingleImage, async (req, res, next) => {
    try {
        const kind = compact(req.params.kind).toLowerCase();
        const targetField = getDraftingSettingsFieldForKind(kind);
        if (!targetField) {
            return res.status(400).json({
                ok: false,
                error: "Invalid asset kind. Use header, footer, letterhead, or signature.",
            });
        }
        ...
        const previousUrl = existing && targetField in existing
        ...
        const settings = await prisma.firmSettings.upsert({
            ...
            create: {
                userId: req.auth.userId,
                draftingBrandingMode: kind === "letterhead" ? "LETTERHEAD" : "HEADER_FOOTER",
                [targetField]: publicUrl,
            },
            update: {
                [targetField]: publicUrl,
                ...(kind === "letterhead"
                    ? { draftingBrandingMode: "LETTERHEAD" }
                    : {}),
            },
        });
...
draftingRouter.delete("/settings/assets/:kind", async (req, res, next) => {
    try {
        const kind = compact(req.params.kind).toLowerCase();
        const targetField = getDraftingSettingsFieldForKind(kind);
        if (!targetField) {
        ...
        const previousUrl = targetField in existing
            ? existing[targetField]
            : null;
...
draftingRouter.get("/templates", async (req, res, next) => {
    try {
...
draftingRouter.get("/templates/:id", async (req, res, next) => {
    try {
...
draftingRouter.post("/templates", async (req, res, next) => {
    try {
        ...
        const template = await prisma.draftTemplate.create({
            data: {
                ...
                precedentStrength: req.body?.precedentStrength === "STRONG" ||
                ...
                placeholdersJson: toNullableJsonInput(Array.isArray(req.body?.placeholders) ? req.body.placeholders : []),
                clauseBlocksJson: toNullableJsonInput(Array.isArray(req.body?.clauseBlocks) ? req.body.clauseBlocks : []),
                executionRequirementsJson: toNullableJsonInput(req.body?.executionRequirements &&
...
draftingRouter.get("/documents", async (req, res, next) => {
    try {
        const family = compact(req.query.family);
...
draftingRouter.post("/documents", async (req, res, next) => {
    try {
        const title = compact(req.body?.title);
        ...
        const conversationId = await resolveOwnedConversationId(req, req.body?.conversationId);
        const sourceTemplateIds = Array.isArray(req.body?.sourceTemplateIds)
            ? req.body.sourceTemplateIds
            : [];
        const inputDataJson = req.body?.inputData && typeof req.body.inputData === "object"
            ? req.body.inputData
            : null;
        ...
        const editorJson = req.body?.editorJson && typeof req.body.editorJson === "object"
            ? req.body.editorJson
            : null;
        const unresolvedPlaceholders = Array.isArray(req.body?.unresolvedPlaceholders)
        ...
        const created = await prisma.$transaction(async (tx) => {
            const document = await tx.draftDocument.create({
                data: {
                    ...
                    status: req.body?.status === "FINAL" || req.body?.status === "ARCHIVED"
                        ? req.body.status
                        : "DRAFT",
                },
            });
            if (createInitialVersion) {
                await tx.draftDocumentVersion.create({
                    data: {
                        ...
                        sourceTemplateIdsJson: toNullableJsonInput(document.sourceTemplateIdsJson),
                        ...
                        unresolvedPlaceholdersJson: toNullableJsonInput(document.unresolvedPlaceholdersJson),
...
draftingRouter.get("/documents/:id", async (req, res, next) => {
    try {
...
draftingRouter.post("/uploads", async (req, res, next) => {
    try {
        ...
        const attachment = await prisma.draftAttachment.create({
            data: {
                ...
                extractedText: text,
                parsedJson: toNullableJsonInput(req.body?.parsedJson &&
...
draftingRouter.get("/uploads", async (req, res, next) => {
    try {
        const conversationId = compact(req.query.conversationId);
...
draftingRouter.get("/uploads/:id", async (req, res, next) => {
    try {
...
draftingRouter.post("/uploads/:id/save-as-template", async (req, res, next) => {
    try {
        const attachment = await prisma.draftAttachment.findFirst({
            where: {
                id: req.params.id,
        ...
        const parsed = parseJsonField(attachment.parsedJson, {});
        const title = compact(req.body?.title) ||
        ...
        const family = compact(req.body?.family) ||
            ...
            "misc";
        const subtype = compact(req.body?.subtype) ||
            (typeof parsed.subtype === "string" ? compact(parsed.subtype) : "") ||
            null;
        const summary = compact(req.body?.summary) ||
...
draftingRouter.patch("/documents/:id", async (req, res, next) => {
    try {
        ...
        if (req.body?.title !== undefined)
            updateData.title = compact(req.body.title);
        if (req.body?.family !== undefined)
            updateData.family = compact(req.body.family) || "misc";
        if (req.body?.subtype !== undefined)
            updateData.subtype = compact(req.body.subtype) || null;
        if (req.body?.strategy !== undefined)
            updateData.strategy = compact(req.body.strategy) || null;
        if (req.body?.matchLevel !== undefined)
        ...
        if (req.body?.editorJson !== undefined) {
            updateData.editorJson = toNullableJsonInput(req.body.editorJson && typeof req.body.editorJson === "object"
                ? req.body.editorJson
                : null);
        }
        ...
        if (req.body?.sourceTemplateIds !== undefined) {
            updateData.sourceTemplateIdsJson = toNullableJsonInput(Array.isArray(req.body.sourceTemplateIds)
                ? req.body.sourceTemplateIds
                : []);
        }
        if (req.body?.inputData !== undefined) {
            updateData.inputDataJson = toNullableJsonInput(req.body.inputData && typeof req.body.inputData === "object"
                ? req.body.inputData
                : null);
        }
        if (req.body?.draftingPlan !== undefined) {
            updateData.draftingPlanJson = toNullableJsonInput(req.body.draftingPlan && typeof req.body.draftingPlan === "object"
                ? req.body.draftingPlan
                : null);
        }
        if (req.body?.unresolvedPlaceholders !== undefined) {
            updateData.unresolvedPlaceholdersJson = toNullableJsonInput(Array.isArray(req.body.unresolvedPlaceholders)
                ? req.body.unresolvedPlaceholders
                : []);
        }
        else {
            const markdownForPlaceholderScan = typeof updateData.draftMarkdown === "string"
                ? updateData.draftMarkdown
                : existing.draftMarkdown || "";
            updateData.unresolvedPlaceholdersJson = toNullableJsonInput(extractUnresolvedPlaceholders(String(markdownForPlaceholderScan || "")));
        }
...
//   }
// });
draftingRouter.post("/documents/:id/regenerate-section", async (req, res, next) => {
    try {
...
draftingRouter.post("/documents/:id/versions", async (req, res, next) => {
    try {
        const document = await prisma.draftDocument.findFirst({
            where: {
                id: req.params.id,
        ...
        const nextVersionNumber = (latestVersion?.versionNumber || 0) + 1;
        const nextTitle = req.body?.title !== undefined ? compact(req.body.title) : document.title;
        const nextFamily = req.body?.family !== undefined
            ? compact(req.body.family) || "misc"
            : document.family;
        const nextSubtype = req.body?.subtype !== undefined
            ? compact(req.body.subtype) || null
            : document.subtype;
        const nextStrategy = req.body?.strategy !== undefined
            ? compact(req.body.strategy) || null
            : document.strategy;
        const nextMatchLevel = req.body?.matchLevel !== undefined
            ? compact(req.body.matchLevel) || null
            : document.matchLevel;
        const nextSourceTemplateIds = req.body?.sourceTemplateIds !== undefined
            ? Array.isArray(req.body.sourceTemplateIds)
                ? req.body.sourceTemplateIds
                : []
            : document.sourceTemplateIdsJson;
        const nextInputData = req.body?.inputData !== undefined
            ? req.body.inputData && typeof req.body.inputData === "object"
                ? req.body.inputData
                : null
            : document.inputDataJson;
        const nextDraftingPlan = req.body?.draftingPlan !== undefined
            ? req.body.draftingPlan && typeof req.body.draftingPlan === "object"
                ? req.body.draftingPlan
                : null
            : document.draftingPlanJson;
        const nextDraftMarkdown = req.body?.draftMarkdown !== undefined
            ? typeof req.body.draftMarkdown === "string"
                ? req.body.draftMarkdown
                : null
            : document.draftMarkdown;
        const nextDraftHtml = req.body?.draftHtml !== undefined
            ? typeof req.body.draftHtml === "string"
                ? req.body.draftHtml
                : null
            : document.draftHtml;
...
draftingRouter.post("/documents/:id/fill-fields", async (req, res, next) => {
    try {
        const document = await prisma.draftDocument.findFirst({
            where: {
                id: req.params.id,
            ...
            const updatedDocument = await tx.draftDocument.update({
                ...
                data: {
                    ...
                    inputDataJson: toNullableJsonInput({
                        ...parseJsonField(document.inputDataJson, {}),
                        filledValues: values,
                    }),
                },
            });
...
draftingRouter.post("/export/pdf", async (req, res, next) => {
    try {
...
draftingRouter.post("/export/docx", async (req, res, next) => {
    try {
        const title = compact(req.body?.title) || "Draft";
        const bodyHtml = typeof req.body?.bodyHtml === "string" ? req.body.bodyHtml : "";
        const branding = req.body?.branding && typeof req.body.branding === "object"
...
draftingRouter.post("/speech/transcribe", speechUpload, async (req, res, next) => {
    try {
        const uploadedFile = req.file;
        if (!uploadedFile) {
            return res.status(400).json({
        ...
        const prompt = compact(req.body?.prompt) ||
        ...
        const result = await transcribeAudioBuffer({
            ...
            fileName: compact(uploadedFile.originalname) || `drafting-voice-${Date.now()}.webm`,
