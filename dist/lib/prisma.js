...
import { buildMssqlConfig, cleanConnectionString, getMssqlSchema, } from "./mssqlConfig.js";
const globalForPrisma = globalThis;
function buildPrismaAdapter() {
    const rawUrl = cleanConnectionString(process.env.DATABASE_URL);
    const schema = getMssqlSchema();
    if (!rawUrl) {
        return new PrismaMssql(buildMssqlConfig({
            primaryPrefix: "DATABASE",
            fallbackPrefix: "SQL",
        }), { schema });
    }
    if (!rawUrl.startsWith("sqlserver://")) {
        throw new Error("Invalid DATABASE_URL. This backend now uses SQL Server only; use sqlserver://...");
    }
