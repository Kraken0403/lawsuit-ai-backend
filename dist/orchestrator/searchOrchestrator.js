...
import { classifyQuery, detectMetadataField, } from "../query/classify.js";
...
import { fetchAllChunksForCase, fetchPreviewChunksForCase, } from "../qdrant/caseReconstruction.js";
...
function sortLatestCaseGroups(groups) {
...
function looksLikePureCitationQuery(query) {
    const q = compact(query);
    if (!q)
        return false;
    if (q.split(/\s+/).length > 8)
        return false;
    return (/\b(19|20)\d{2}\b/.test(q) &&
        /\b\d{1,5}\b/.test(q) &&
        /\b(AIR|AIR\s*\(SCW\)|SCW|SCC|SCR|Cri\s*LJ|CrLJ|LawSuit)\b/i.test(q));
}
function inferCitationCourtFilters(query) {
    const q = normalizeLooseSimple(query);
    if (/\bsc\b|\bsupreme court\b/.test(q))
        return ["Supreme Court"];
    if (/\bguj\b|\bgujarat\b/.test(q))
        return ["Gujarat High Court"];
    if (/\bdel\b|\bdelhi\b/.test(q))
        return ["Delhi High Court"];
    if (/\bbom\b|\bbombay\b|\bmumbai\b/.test(q))
        return ["Bombay High Court"];
    return [];
}
function buildFastCitationClassifiedQuery(rawQuery) {
...
function getGroupDecisionTime(group) {
    if (!group)
        return null;
...
function titleMatchStrength(title, target) {
...
function getCaseTargetTokensLoose(caseTarget) {
    if (!caseTarget)
        return [];
    const stop = new Set([
        "case",
...
function groupContainsCitation(group, citations = []) {
    if (!group || !citations.length)
        return false;
    const candidates = new Set();
    if (group.citation)
        candidates.add(group.citation);
    for (const eq of getGroupEquivalentCitations(group))
        candidates.add(eq);
    const candidateFingerprints = new Set([...candidates].map(citationTokenFingerprint).filter(Boolean));
    for (const citation of citations) {
        const fp = citationTokenFingerprint(citation);
        if (fp && candidateFingerprints.has(fp))
            return true;
    }
    return false;
}
function isStrongResolvedMatch(group, classified) {
    if (!group || !classified.caseTarget)
        return false;
    return titleMatchStrength(group.title || "", classified.caseTarget) >= 2;
}
function isExactCitationResolved(group, classified) {
    if (!classified.citations?.length)
        return false;
    return groupContainsCitation(group, classified.citations);
}
function isAcceptableResolvedMatch(group, classified) {
    if (!group)
        return false;
    if (isExactCitationResolved(group, classified)) {
        return true;
    }
    if (!classified.caseTarget)
        return false;
    ...
    if (coverage >= 0.5 && bestScore >= 20)
        return true;
    if (classified.metadataField === "citation" && coverage >= 0.5)
        return true;
    return false;
}
...
function getLatestAssistantCaseDigests(messages = []) {
...
function extractOrdinalMentions(text) {
    const q = ` ${String(text || "").toLowerCase()} `;
    const regex = /\b(first|1st|second|2nd|third|3rd|fourth|4th|fifth|5th|last)\s+(case|one|judgment|decision)\b/g;
    const out = [];
    let match;
    while ((match = regex.exec(q))) {
        const label = match[1];
        let value = null;
        if (label === "first" || label === "1st")
            value = 1;
        else if (label === "second" || label === "2nd")
            value = 2;
        else if (label === "third" || label === "3rd")
            value = 3;
        else if (label === "fourth" || label === "4th")
            value = 4;
        else if (label === "fifth" || label === "5th")
            value = 5;
        else if (label === "last")
            value = -1;
...
function resolveOrdinalDigest(digests, ordinal) {
    if (!digests.length)
        return null;
...
function isSummaryQuery(query) {
    const q = query.toLowerCase();
    return (q.includes("summarize") ||
        q.includes("summarise") ||
        q.includes("summary") ||
        q.includes("brief"));
}
function isHoldingQuery(query) {
    const q = query.toLowerCase();
    return (q.includes("holding") ||
        q.includes("ratio decidendi") ||
        q.includes("ratio") ||
        q.includes("what did the court hold") ||
        q.includes("what was held"));
}
function isFullJudgmentQuery(query) {
    const q = query.toLowerCase();
    return (q.includes("full judgment") ||
        q.includes("full text") ||
        q.includes("complete judgment") ||
        q.includes("entire judgment") ||
        q.includes("judgment text"));
}
function isMetadataReferenceQuery(query) {
    const q = query.toLowerCase();
    return (q.includes("citation") ||
        q.includes("court") ||
        q.includes("judges") ||
        q.includes("judge") ||
...
function stripMetadataInstruction(text) {
    let out = compact(text);
    if (!out)
        return "";
    out = out.replace(/^(give|find|show|tell me|what is|who were|who are)\s+/i, "");
    out = out.replace(/^(the\s+)?(citation|court|judges?|judge|date of decision|decision date|case number|case no|acts referred|subject|final decision|advocates|case type)\s+(of|for)\s+/i, "");
    out = out.replace(/\s+\b(citation|court|judges?|judge|date of decision|decision date|case number|case no|acts referred|subject|final decision|advocates|case type)\b\s*$/i, "");
    return compact(out);
}
function getCaseTargetVariants(classified) {
    const rawCandidates = [
...
function rewriteForSingleTarget(query, target) {
    ...
    if (isMetadataReferenceQuery(query)) {
        if (query.toLowerCase().includes("citation"))
            return `Give the citation of ${ref}`;
        if (query.toLowerCase().includes("court"))
            return `What is the court in ${ref}`;
        if (query.toLowerCase().includes("judges") || query.toLowerCase().includes("judge")) {
            return `Who were the judges in ${ref}`;
        }
        if (query.toLowerCase().includes("date of decision") ||
...
function maybeRewriteFollowUpQuery(query, messages = []) {
    const digests = getLatestAssistantCaseDigests(messages);
    if (!digests.length)
        return query;
    const q = compact(query);
    ...
    const deicticReference = /\b(that case|this case|that judgment|this judgment|that one|this one|it)\b/.test(lower);
...
function resolveReferenceFromMessages(rawQuery, messages = []) {
    const digests = getLatestAssistantCaseDigests(messages);
    const notes = [];
...
function shouldUseLegacyClassifier(classified) {
    return (classified.intent === "unknown" ||
...
async function resolveCitationAliasWithLlm(classified, hybridHits) {
    if (!citationAliasClient || !classified.citations?.length)
        return null;
    ...
    const relevant = hybridHits
        ...
        return targetCitations.some((target) => text.includes(target) || title.includes(target) || citation.includes(target));
    })
        .slice(0, 8);
    if (!relevant.length)
        return null;
    ...
    try {
        const response = await citationAliasClient.responses.create({
            ...
            input: [
                {
                    role: "system",
                    content: "You extract the actual case title corresponding to a legal citation from quoted legal excerpts. Return null if the excerpts do not explicitly identify the case. Do not guess from the surrounding hit title unless the excerpt itself makes the mapping clear.",
                },
                {
                    role: "user",
                    content: [
...
function latestSearchBlob(group) {
    return [
        metadataBlob(group),
...
function filterLatestCaseGroups(groups, classified) {
    if (!groups.length)
        return [];
    ...
    const strict = groups.filter((group) => {
        ...
        const subjectOk = !subjectKeywords.length ||
            subjectKeywords.some((s) => searchBlob.includes(s));
        const jurisdictionOk = !jurisdictions.length ||
            jurisdictions.some((j) => matchesJurisdictionConstraint(group, j));
        const courtOk = !courts.length || courts.some((c) => matchesCourtConstraint(group, c));
    ...
    const newestSorted = [...candidateSet].sort((a, b) => {
        ...
        if (forumBoostB !== forumBoostA)
            return forumBoostB - forumBoostA;
        return (b.bestScore || 0) - (a.bestScore || 0);
    });
    if (!newestSorted.length)
        return [];
    const newestTime = getGroupDecisionTime(newestSorted[0]);
    if (!newestTime)
        return newestSorted.slice(0, 5);
...
function buildLatestEvidence(groups, max = 10) {
...
function shouldExpandWholeCase(classified, mode = "hybrid") {
    if (mode === "full_judgment")
        return true;
    if (classified.intent === "full_judgment")
        return true;
    return false;
}
function buildCitationNotFoundResult(classified, trace, mode = "hybrid") {
    return {
        query: classified,
        mode,
        groupedCases: [],
...
async function runHybridPath(classified, trace, mode = "hybrid") {
    const hybridHits = dedupeChunks(await runHybridSearch(classified, classified.intent === "latest_cases" ? 40 : undefined));
    console.log("[runHybridPath] hybridHits:", hybridHits.length);
    console.log("[runHybridPath] firstHitMeta:", hybridHits.slice(0, 3).map((hit) => ({
        caseId: hit.caseId,
        title: hit.title,
    ...
    let groupedCases = groupHitsByCase(hybridHits, classified.intent === "latest_cases" ? 5 : 3, classified);
    console.log("[runHybridPath] groupedCases:", groupedCases.length);
    console.log("[runHybridPath] groupedMeta:", groupedCases.slice(0, 5).map((group) => ({
        caseId: group.caseId,
        title: group.title,
    ...
    if (classified.intent === "latest_cases") {
        ...
        if (filtered.length) {
            ...
            trace.notes.push("applied latest-case strict filtering with chunk-text subject matching");
        }
        else {
            trace.notes.push("latest-case strict filtering found no stronger narrowed set; kept original groups");
        }
...
async function resolveByCitationFirst(classified, trace, mode = "hybrid") {
    if (!classified.citations?.length)
        return null;
    ...
    const chunks = shouldExpandWholeCase(classified, mode)
        ...
        : metadataHits
            ...
            .slice(0, 6);
    trace.notes.push(shouldExpandWholeCase(classified, mode)
...
async function resolveByCitationAliasRecovery(classified, trace, mode = "hybrid") {
    if (!classified.citations?.length)
        return null;
    ...
    if (!aliasTitle) {
        trace.notes.push("citation alias recovery found no resolvable case-title alias in retrieved citation-bearing snippets");
        return null;
    }
    const perTarget = {
        ...classified,
    ...
    if (!hybridTop || !isAcceptableResolvedMatch(hybridTop, hybridPerTarget)) {
        trace.notes.push(`citation alias recovery extracted "${aliasTitle}" but both metadata and hybrid title resolution were weak`);
        return null;
    }
    const chunks = shouldExpandWholeCase(classified, mode)
        ? mode === "full_judgment"
...
async function resolveComparisonTargetsFirst(classified, trace) {
...
async function resolveNamedCaseAndExpand(classified, trace, mode = "hybrid") {
    if (!classified.caseTarget)
        return null;
    const metadataHits = dedupeChunks(await runMetadataSearch(classified, 8));
    const metadataGroups = groupHitsByCase(metadataHits, 1, classified);
...
async function resolveMetadataLookupByTargetFallback(classified, trace) {
...
export async function orchestrateSearch(input) {
    ...
    if (classified.citations?.length) {
        const resolvedCitation = await resolveByCitationFirst(classified, trace, classified.intent === "full_judgment" ? "full_judgment" : "hybrid");
        if (resolvedCitation)
            return resolvedCitation;
        const recoveredCitationAlias = await resolveByCitationAliasRecovery(classified, trace, classified.intent === "full_judgment" ? "full_judgment" : "hybrid");
        if (recoveredCitationAlias)
            return recoveredCitationAlias;
    ...
    if (classified.intent === "metadata_lookup") {
        ...
        trace.notes.push("metadata lookup returned no strong resolved match; trying target-only dense rescue");
        ...
        const hybridFallback = await runHybridPath({
            ...classified,
            strategy: classified.metadataField === "citation"
                ? "dense_heavy"
                : classified.strategy === "citation_heavy"
                    ? "balanced"
                    : classified.strategy,
        }, trace, "hybrid");
        if (hybridFallback.groupedCases.length > 0 &&
    ...
    if (classified.intent === "latest_cases") {
        trace.notes.push("router identified latest/recent case query; shared court resolution, runtime date parsing, state-aware filtering, and newest-first ordering are active");
    }
    const resolvedHybrid = await resolveNamedCaseAndExpand(classified, trace, "hybrid");
    if (resolvedHybrid)
        return resolvedHybrid;
