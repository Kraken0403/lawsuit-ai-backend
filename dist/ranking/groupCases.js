...
function scoreCaseGroup(group, query) {
    ...
    if (query.citations?.length) {
        for (const citation of query.citations) {
            ...
            if (citationText === c)
                score += 5.0;
            else if (citationText.includes(c) || c.includes(citationText))
                score += 2.2;
        }
    }
    ...
    if (query.intent === "latest_cases" || query.strategy === "recency_heavy") {
        ...
        if (bestDecisionTime) {
            const ageDays = Math.max(0, (Date.now() - bestDecisionTime) / (1000 * 60 * 60 * 24));
            if (ageDays <= 30)
                score += 1.2;
            else if (ageDays <= 180)
                score += 0.85;
...
export function groupHitsByCase(hits, maxChunksPerCase = 2, query) {
    ...
    const sorted = [...grouped.values()].sort((a, b) => scoreCaseGroup(b, query) - scoreCaseGroup(a, query));
    if (query?.caseTarget && isDirectCaseLookupIntent(query.intent)) {
        const strong = sorted.filter((g) => titleMatchStrength(g.title || "", query.caseTarget) >= 2);
        if (strong.length > 0) {
            const rest = sorted.filter((g) => titleMatchStrength(g.title || "", query.caseTarget) < 2);
            return [...strong, ...rest];
        }
    }
    return sorted;
}
//# sourceMappingURL=groupCases.js.map