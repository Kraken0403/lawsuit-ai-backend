...
function toCanonicalCourt(entry, rawCourt) {
    return {
        code: entry.code,
        level: entry.level,
        state: entry.state,
...
function findCourtEntriesByText(input) {
    const wanted = normalizeCourtText(input);
    if (!wanted)
        return [];
    return Object.values(COURT_ID_MAP).filter((entry) => entry.aliases.some((alias) => {
        const normalizedAlias = normalizeCourtText(alias);
        return (normalizedAlias === wanted ||
            normalizedAlias.includes(wanted) ||
            wanted.includes(normalizedAlias));
    }));
}
function inferCourtFromCourtId(payload) {
    const rawCourt = compact([String(payload.court ?? ""), String(payload.courtName ?? "")]
        .filter(Boolean)
        .join(" "));
...
export function getCourtIdsForFilter(courtFilter) {
...
const HIGH_COURT_PATTERNS = Object.values(COURT_ID_MAP)
    .filter((entry) => entry.level === "HC" && typeof entry.state === "string" && entry.state.length > 0)
    .map((entry) => ({
    code: entry.code,
    state: entry.state,
    aliases: entry.aliases,
}));
const COURT_CODE_TO_IDS = Object.values(COURT_ID_MAP).reduce((acc, entry) => {
    if (!acc[entry.code])
        acc[entry.code] = [];
...
const STATE_TO_HC = Object.fromEntries(HIGH_COURT_PATTERNS.map((item) => [item.state, { code: item.code, state: item.state }]));
const REPORTER_CODE_MAP = {
...
function normalizeReporterCode(code) {
    return code.toLowerCase().replace(/[^a-z0-9]/g, "");
}
function extractReporterCodes(text) {
...
function inferCourtFromReporterValues(values) {
...
function inferCourtFromPrefix(payload) {
...
function inferCourtFromText(rawCourt) {
...
export function canonicalizeJurisdictionName(input) {
    const normalized = normalizeLooseSimple(input);
    return STATE_ALIAS_MAP[normalized] || normalized;
}
export function normalizeLatestJurisdictions(classified) {
...
export function canonicalizeCourt(payload) {
    ...
    const citationValues = [
        ...
        ...((Array.isArray(payload.equivalentCitations) ? payload.equivalentCitations : []).map((x) => String(x))),
    ].filter(Boolean);
...
function getGroupCourts(group) {
    const out = [];
    const seen = new Set();
    for (const chunk of group.chunks || []) {
        const resolved = canonicalizeCourt((chunk.payload || {}));
        const key = `${resolved.code || ""}|${resolved.level || ""}|${resolved.state || ""}|${normalizeCourtText(resolved.rawCourt)}`;
        if (!seen.has(key)) {
            seen.add(key);
            out.push(resolved);
        }
...
export function matchesCourtConstraint(group, courtFilter) {
...
export function matchesJurisdictionConstraint(group, jurisdiction) {
    const target = canonicalizeJurisdictionName(jurisdiction);
    if (!target || target === "india")
        return true;
    const groupCourts = getGroupCourts(group);
...
export function getLatestForumBoost(group, classified) {
    ...
    if (!resolvedCourts.length)
        return 0;
    if (requestedCourts.length) {
        const matched = resolvedCourts.some((resolved) => requestedCourts.some((requested) => requested.code === resolved.code));
...
//# sourceMappingURL=courtResolver.js.map