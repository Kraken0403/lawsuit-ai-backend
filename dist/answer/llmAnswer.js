...
import { selectSingleCaseEvidence, buildChunkBatches, citationFromChunk, } from "./caseEvidenceSelector.js";
function getClient() {
...
function getReasoningConfig(model, searchResult) {
    ...
    const isFollowUp = Boolean(searchResult?.trace?.router?.isFollowUp || searchResult?.query?.followUpLikely);
    const groupedCases = Array.isArray(searchResult?.groupedCases)
        ? searchResult.groupedCases
        : [];
    const singleHugeCase = groupedCases.length === 1 &&
        Array.isArray(groupedCases[0]?.chunks) &&
        groupedCases[0].chunks.length > 20;
    const complex = isFollowUp ||
        intent === "comparison" ||
...
function answerStyle(searchResult) {
    const intent = searchResult?.query?.intent || "unknown";
    const q = (searchResult?.trace?.router?.resolvedQuery ||
        searchResult?.query?.originalQuery ||
        searchResult?.query?.normalizedQuery ||
        "").toLowerCase();
    if (intent === "latest_cases")
        return "latest_cases";
    if (intent === "comparison")
        return "comparison";
    if (intent === "holding_search")
        return "holding";
    if (intent === "full_judgment")
...
function chooseFinalModel(searchResult) {
    ...
    const q = (searchResult?.trace?.router?.resolvedQuery ||
        ...
        searchResult?.query?.originalQuery ||
        "").toLowerCase();
    const comparisonLike = searchResult?.query?.intent === "comparison" ||
        q.includes("compare") ||
        q.includes("difference between") ||
        q.includes("distinguish");
    const followUpLike = Boolean(searchResult?.trace?.router?.isFollowUp) ||
        Boolean(searchResult?.query?.followUpLikely);
    const singleHugeCase = Array.isArray(searchResult?.groupedCases) &&
    ...
    const holdingLike = searchResult?.query?.intent === "holding_search" ||
        q.includes("holding") ||
        q.includes("ratio decidendi") ||
...
function buildResolvedReferenceText(searchResult) {
    ...
    if (!resolved || !resolved.usedPriorContext)
        return "";
...
function buildQueryContextText(searchResult) {
    const query = searchResult?.trace?.router?.resolvedQuery ||
        searchResult?.query?.originalQuery ||
        searchResult?.query?.normalizedQuery ||
        "";
...
function renderBatch(group, batchChunks) {
    ...
    for (const chunk of batchChunks) {
        const para = chunk.paragraphStart != null && chunk.paragraphEnd != null
...
async function summarizeSingleCase(client, searchResult) {
    ...
    const selected = selectSingleCaseEvidence(group, searchResult?.trace?.router?.resolvedQuery ||
        searchResult?.query?.originalQuery ||
        searchResult?.query?.normalizedQuery ||
        "");
    if (!selected.selectedChunks.length)
        return null;
    ...
    for (let i = 0; i < batches.length; i += 1) {
        ...
        if (memo) {
            ...
            const citation = citationFromChunk(group, batchChunks[Math.floor(batchChunks.length / 2)] || first);
            batchMemos.push({
                memo,
...
export async function generateLlmAnswer(searchResult) {
    ...
    if (!client)
        return null;
    try {
        let groupedCases = reorderGroupedCasesFromTrace(Array.isArray(searchResult.groupedCases) ? searchResult.groupedCases : [], searchResult);
        ...
        const singleResolvedCase = Array.isArray(effectiveSearchResult.groupedCases) &&
        ...
        for (const group of (effectiveSearchResult.groupedCases || []).slice(0, 5)) {
            ...
            if (chunk)
                usedCitations.push(citationFromChunk(group, chunk));
        }
        return {
            model,
            text,
...
//# sourceMappingURL=llmAnswer.js.map