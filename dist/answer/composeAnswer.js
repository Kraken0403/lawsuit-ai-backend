...
function citationResolutionPenalty(searchResult, warningsOut) {
    ...
    const aliasResolved = notes.some((note) => String(note || "").toLowerCase().includes("citation alias llm recovery resolved"));
    if (!exactResolved && !aliasResolved) {
        if (notes.some((note) => String(note || "").toLowerCase().includes("citation-first resolution found no exact citation match"))) {
            penalty += 0.18;
            warningsOut.push("Direct citation resolution was not exact; related case cards may be suppressed to avoid showing the wrong case.");
        }
    }
    if (aliasResolved) {
        penalty += 0.06;
...
function shouldSuppressCaseCards(searchResult, llmText) {
    const notes = Array.isArray(searchResult?.trace?.notes) ? searchResult.trace.notes : [];
    const hasCitationQuery = Array.isArray(searchResult?.query?.citations) && searchResult.query.citations.length > 0;
    if (!hasCitationQuery)
        return false;
    const exactResolved = notes.some((note) => String(note || "").toLowerCase().includes("resolved exact citation and expanded full case"));
    const aliasResolved = notes.some((note) => String(note || "").toLowerCase().includes("citation alias llm recovery resolved"));
    if (exactResolved || aliasResolved)
        return false;
    const failedResolution = notes.some((note) => {
        const n = String(note || "").toLowerCase();
        return (n.includes("citation-first resolution found no exact citation match") ||
...
function deriveWarnings(searchResult, llmText) {
    ...
    if (shouldSuppressCaseCards(searchResult, llmText)) {
        warnings.push("Related case cards were hidden because citation resolution did not confidently identify the actual case.");
    }
...
//# sourceMappingURL=composeAnswer.js.map