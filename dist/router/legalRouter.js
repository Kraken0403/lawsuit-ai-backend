...
import { ROUTER_SYSTEM_PROMPT, ROUTER_VERSION, assertRouterOutput, routerJsonSchema, } from "./routerSchema.js";
...
function buildFallbackRouterOutput(query) {
    return {
...
function hasExplicitFollowUpCue(query) {
    const q = query.toLowerCase();
    return (/\b(that case|this case|that judgment|this judgment|that one|this one|it|same case|same judgment|the above case|the above judgment|former|latter)\b/.test(q) ||
        /\b(first|1st|second|2nd|third|3rd|fourth|4th|fifth|5th|last)\s+(case|one|judgment|decision)\b/.test(q) ||
        /\b(compare them|compare those|difference between them|distinguish them)\b/.test(q));
}
function queryReferencesConversationState(query, state) {
    if (!state)
        return false;
    const q = normalizeLoose(query);
    for (const title of state.activeCaseTitles || []) {
        const t = normalizeLoose(title);
        if (t && (q.includes(t) || t.includes(q)))
            return true;
    }
...
function routerCarriesStateInheritance(router) {
...
function routerLooksLikeFreshQuery(router) {
    return Boolean(router.entities.caseTarget ||
...
//# sourceMappingURL=legalRouter.js.map