...
function isRetryableQdrantError(error) {
...
function getCaseTargetTokens(caseTarget) {
    if (!caseTarget)
        return [];
    const stop = new Set([
        "case",
    ...
    return unique(normalizeLoose(caseTarget)
        .split(" ")
        .filter((tok) => tok.length >= 3 && !stop.has(tok)));
}
function getCitationTokens(citation) {
    if (!citation)
        return [];
    return unique(normalizeLoose(citation)
        .split(" ")
        .filter((tok) => tok.length >= 2));
}
function getCitationVariants(citation) {
...
function extractCaseTypeHints(classified) {
...
function mergeFilters(baseFilter, filter) {
    if (!baseFilter)
        return filter;
    const merged = {};
    ...
    if (must.length)
        merged.must = must;
    if (should.length)
        merged.should = should;
    if (must_not.length)
        merged.must_not = must_not;
    return merged;
}
function getMetadataSearchPlan(classified) {
...
function buildMetadataFilters(classified) {
    const ranked = [];
    const seen = new Set();
    const baseFilter = buildQdrantPayloadFilter(classified);
    const pushFilter = (priority, filter) => {
...
async function runScroll(filter, opts = {}) {
    ...
    while (points.length < pointLimit && pages < maxPages) {
        ...
        for (let attempt = 1; attempt <= 3; attempt += 1) {
            try {
                ...
                console.warn(`[metadataSearch] qdrant.scroll retry ${attempt}/3 after ${waitMs}ms:`, error?.cause?.code || error?.message || error);
                await sleep(waitMs);
            }
        }
        if (!response) {
...
function scoreMetadataHit(hit, classified) {
    ...
    const citedCount = typeof payload.cited === "number" ? payload.cited : Number(payload.cited || 0);
    const requestedCourtCodes = getRequestedCourtCodes(classified);
    const hitCourtCode = canonicalizeCourt(payload).code;
    let score = 0;
    const caseTargetVariants = getCaseTargetVariants(classified);
    ...
    for (const rawCitation of classified.citations || []) {
        for (const citationQuery of getCitationVariants(rawCitation)) {
            ...
            if (normalizeLoose(citation) === queryLoose)
                score += 220;
            if (equivalentCitations.some((c) => normalizeLoose(c) === queryLoose))
                score += 190;
            if (includesLoose(citation, citationQuery))
                score += 40;
            if (equivalentCitations.some((c) => includesLoose(c, citationQuery)))
                score += 35;
            if (includesLoose(hit.text || "", citationQuery))
                score += 48;
            ...
            const tokenHits = citationTokens.filter((t) => citationLoose.includes(t)).length +
    ...
    if (requestedCourtCodes.length) {
        ...
        for (const courtFilter of filters.courts || []) {
            if (includesLoose(court, courtFilter))
                score += 28;
        }
    }
    for (const subjectFilter of filters.subjects || []) {
        if (includesLoose(subject, subjectFilter))
            score += 24;
    }
    for (const statute of filters.statutes || []) {
        if (arrayIncludesLoose(actsReferred, statute))
            score += 18;
    }
    for (const section of filters.sections || []) {
        if (arrayIncludesLoose(actsReferred, section))
            score += 18;
    }
    for (const hint of caseTypeHints) {
        if (includesLoose(caseType, hint))
            score += 20;
    }
    if (classified.metadataField) {
        switch (classified.metadataField) {
            ...
            case "equivalentCitations":
                if (citation || equivalentCitations.length)
                    score += 10;
                break;
            default:
                break;
...
export async function runMetadataSearch(classified, limit = 24) {
    const rankedFilters = buildMetadataFilters(classified);
    if (rankedFilters.length === 0)
        return [];
    ...
    for (let i = 0; i < filtersToRun.length; i += plan.concurrency) {
        ...
        const settled = await Promise.allSettled(batch.map((entry) => runScroll(entry.filter, {
        ...
        for (const result of settled) {
            if (result.status !== "fulfilled") {
                console.warn("[metadataSearch] filter batch failed:", result.reason?.message || result.reason);
                continue;
            }
