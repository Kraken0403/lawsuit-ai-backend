...
import { buildQdrantPayloadFilter, getRequestedCourtCodes, } from "./payloadFilters.js";
...
function isDirectCaseLookupIntent(intent) {
    return (intent === "case_lookup" ||
        intent === "full_judgment" ||
        intent === "metadata_lookup");
}
function toChunkHit(result) {
    ...
    return {
        ...
        paragraphStart: payload.paragraphStart != null ? Number(payload.paragraphStart) : null,
        paragraphEnd: payload.paragraphEnd != null ? Number(payload.paragraphEnd) : null,
        text: String(payload.text || ""),
        payload,
    };
}
function getPrefetchLimits(query) {
...
function shouldSkipDense(query) {
    return (query.strategy === "citation_heavy" &&
        !query.caseTarget &&
...
function extractCaseTypeHints(query) {
...
function containsAnyLoose(text, patterns) {
    const hay = normalizeLoose(text);
    return patterns.some((pattern) => hay.includes(normalizeLoose(pattern)));
}
function getIssueConceptGroups(query) {
    if (query.intent !== "issue_search")
        return [];
    const q = normalizeLoose(query.normalizedQuery || query.originalQuery || "");
    const lowerCourtHints = (query.filters?.lowerCourtHints || []).map((x) => normalizeLoose(x));
    const groups = [];
    const hasFakeCitation = /\bnon[\s-]?existent\b|\bdoes not exist\b|\bnot exist\b|\bfabricated\b|\bfalse citation\b|\bfake citation\b|\bwrong citation\b|\bbogus\b|\bimaginary\b|\bforged\b|\bfraud on court\b/.test(q);
    const hasSupremeCourt = /\bsupreme court\b|\bsc\b/.test(q);
    const hasLowerCourt = /\bcivil court\b|\btrial court\b|\bsubordinate court\b|\bcivil judge\b|\bdistrict judge\b|\bsessions court\b/.test(q) || lowerCourtHints.length > 0;
    const hasAction = /\baction\b|\bdisciplinary\b|\bstrictures?\b|\badverse remarks?\b|\bproceedings against\b|\bmisconduct\b|\breprimand\b|\bsupervisory\b/.test(q);
    if (hasFakeCitation) {
        groups.push({
            patterns: [
                "non existent",
...
function scoreIssueSearchConcepts(hit, query) {
...
function expandOriginAliases(term) {
...
function scoreOriginStateForTransferredOrAppealedCase(hit, query) {
    const rawTerms = [
    ...
    const hay = normalizeLoose([court, courtName, state, jurisdiction, title, text].join(" "));
...
function expandLowerCourtHintPatterns(lowerCourtHints) {
    const out = [];
    for (const rawHint of lowerCourtHints) {
        const hint = normalizeLoose(rawHint);
        if (hint.includes("civil court")) {
            out.push("civil court", "principal civil judge", "city civil court", "civil judge", "district court", "subordinate court");
            continue;
        }
        if (hint.includes("trial court")) {
            out.push("trial court", "sessions court", "district and sessions judge", "principal sessions judge", "trial judge", "magistrate", "jmfc");
            continue;
        }
        if (hint.includes("subordinate")) {
            out.push("subordinate court", "trial court", "civil judge", "district judge", "sessions judge", "magistrate");
            continue;
        }
        out.push(hint);
    }
...
function scoreLowerCourtHints(hit, query) {
    const lowerHints = query.filters?.lowerCourtHints || [];
    if (!lowerHints.length)
        return 0;
...
function postScoreHit(hit, query) {
    ...
    const hitCourtCode = canonicalizeCourt((hit.payload || {})).code;
    if (query.caseTarget) {
        ...
        if (isDirectCaseLookupIntent(query.intent)) {
            if (titleStrength === 3)
                score += 3.2;
            else if (titleStrength === 2)
                score += 2.2;
            else if (titleStrength === 1)
                score += 0.45;
            else
                score -= 1.85;
    ...
    if (query.citations?.length) {
        ...
        for (const citation of query.citations) {
            ...
            const equivalent = Array.isArray(hit.payload?.equivalentCitations)
                ? hit.payload.equivalentCitations.map((x) => normalize(String(x)))
                : [];
            if (hitCitation === c) {
                score += 4.5;
    ...
    if (query.intent === "holding_search") {
        if (/\bheld\b|\bholding\b|\bwe hold\b|\bit is held\b|\btherefore\b|\bwe conclude\b/i.test(hit.text)) {
            score += 0.16;
        }
    }
    const doctrineStyle = query.intent === "issue_search" &&
        /basic structure|doctrine|evolution|history|jurisprudence|constitutional framework/i.test(query.normalizedQuery);
    if (doctrineStyle) {
        if (/kesavananda/i.test(title))
            score += 0.55;
        if (/golak/i.test(title))
            score += 0.28;
        if (/minerva/i.test(title))
            score += 0.28;
        if (/indira/i.test(title))
            score += 0.24;
        if (/coelho/i.test(title))
            score += 0.2;
        if (/waman/i.test(title))
            score += 0.18;
        const citedCount = typeof hit.payload?.cited === "number"
            ? hit.payload.cited
    ...
    if (query.strategy === "recency_heavy" || query.intent === "latest_cases") {
        ...
        if (decisionTime) {
            const ageDays = Math.max(0, (Date.now() - decisionTime) / (1000 * 60 * 60 * 24));
            if (ageDays <= 30)
                score += 1.4;
            else if (ageDays <= 180)
                score += 1.0;
    ...
    if (normalizedQuery.split(" ").length > 18) {
        if (hit.text.length > 300)
            score += 0.03;
    }
    return score;
}
export async function runHybridSearch(classified, limit) {
    ...
    const prefetch = [sparsePrefetch];
    if (!skipDense && limits.denseLimit > 0) {
        const denseInput = classified.caseTarget && isDirectCaseLookupIntent(classified.intent)
