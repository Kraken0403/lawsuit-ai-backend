...
function inferPlaceholderLabel(raw, context) {
    const trimmed = compact(raw);
    const lower = `${context} ${trimmed}`.toLowerCase();
    if (/(date|dated|day|month|year)/.test(lower))
        return "DATE";
    if (/(amount|sum|total|price|fee|cost|consideration|rent)/.test(lower))
        return "AMOUNT";
    if (/(email|e-mail|mail id)/.test(lower))
        return "EMAIL";
    if (/(phone|mobile|contact|whatsapp|telephone|tel)/.test(lower))
        return "PHONE";
    if (/(address|registered office|residence|residential)/.test(lower))
        return "ADDRESS";
    if (/(name of|client|customer|buyer|seller|vendor|party|recipient|petitioner|respondent|accused|complainant)/.test(lower))
        return "NAME";
    return "DETAILS";
}
...
export function normalizeDraftPlaceholders(markdown) {
    const source = String(markdown || "");
    return source.replace(/\[([^\]\n]{0,120})\]/g, (match, inner, offset) => {
