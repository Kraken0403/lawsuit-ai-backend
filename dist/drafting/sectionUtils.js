...
export function findSectionByHeading(markdown, heading) {
...
export function replaceSectionByHeading(markdown, heading, replacementSectionText) {
...
export function extractHeadingFromSection(sectionText) {
...
export function ensureSameHeadingLevel(originalSection, regeneratedSection) {
    ...
    let body = raw;
    const sameLineHeadingRegex = new RegExp(`^#{1,6}\\s*${escapeRegExp(targetHeading)}\\s*[:\\-]?\\s*`, "i");
    ...
    if (markdownHeadingMatch) {
        ...
        if (regeneratedHeading.toLowerCase() === targetHeading.toLowerCase() ||
...
export function listSectionHeadings(markdown) {
    return getMarkdownSections(markdown).map((section) => section.heading);
}
function scoreSectionMatch(section, sectionKey) {
...
export function findLooseSectionByText(markdown, sectionKey) {
    const sections = getMarkdownSections(markdown);
...
export function normalizeSectionHeadingKey(value) {
