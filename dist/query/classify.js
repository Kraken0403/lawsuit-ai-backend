...
export function detectMetadataField(query) {
    const q = query.toLowerCase();
    if (/\bequivalent citation\b|\bequivalent citations\b/.test(q))
        return "equivalentCitations";
    if (/\bacts referred\b|\bact referred\b/.test(q))
        return "actsReferred";
    if (/\bsubject\b/.test(q))
        return "subject";
    if (/\bfinal decision\b/.test(q))
        return "finalDecision";
    if (/\badvocates\b|\badvocate\b/.test(q))
        return "advocates";
    if (/\bjudges\b|\bjudge\b|\bbench\b/.test(q))
        return "judges";
    if (/\bwhich court\b/.test(q) ||
    ...
    if (/\bcitation\b/.test(q))
        return "citation";
    if (/\bdate of decision\b|\bdate decided\b/.test(q))
        return "dateOfDecision";
    if (/\bcase number\b|\bcase no\b/.test(q))
        return "caseNo";
    if (/\bcase type\b/.test(q))
        return "caseType";
    return null;
}
export function stripMetadataInstruction(text) {
    let out = (text || "").replace(/\s+/g, " ").trim();
    if (!out)
        return "";
    out = out.replace(/^(give|find|show|tell me|what is|what's|who were|who are)\s+/i, "");
    out = out.replace(/^(the\s+)?(citation|court|judges?|judge|date of decision|date decided|decision date|case number|case no|acts referred|subject|final decision|advocates|case type)\s+(of|for)\s+/i, "");
    out = out.replace(/\s+\b(citation|court|judges?|judge|date of decision|date decided|decision date|case number|case no|acts referred|subject|final decision|advocates|case type)\b\s*$/i, "");
...
function isComparisonQuery(query, comparisonTargets) {
    const q = query.toLowerCase();
    return (comparisonTargets.length >= 2 &&
        (q.includes("compare") ||
            q.includes("difference between") ||
            q.includes("distinguish")));
}
function inferIntent(normalizedQuery, caseTarget, metadataField, comparisonTargets) {
    const reasons = [];
    ...
    if (caseTarget)
        reasons.push("case target extracted");
    return {
        intent: "hybrid",
...
export function classifyQuery(query) {
    ...
    const caseTarget = extractCaseTarget(metadataField ? cleanedCitationQuery : normalizedQuery);
    ...
    const citations = CITATION_PATTERNS.flatMap((rx) => findAllMatches(rx, cleanedCitationQuery));
    const { intent, confidence, reasons } = inferIntent(normalizedQuery, caseTarget, metadataField, comparisonTargets);
    const caseHints = comparisonTargets.length > 0
        ? comparisonTargets
        : caseTarget
            ? [caseTarget]
            : citations.length
    ...
    return {
        ...
        comparisonTargets,
        followUpLikely: /\bthis case\b|\bthat case\b|\bthis judgment\b|\bthat judgment\b/i.test(normalizedQuery),
